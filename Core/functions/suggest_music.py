...
import webbrowser
def top_trending():
    speak("I will fetch to top trending songs for you sir", save=False)
...
def play_music(title):
    try:
        speak("I will play the song {} now, enjoy".format(title), save=False)
        print(title)
        webbrowser.open("https://open.spotify.com/search/" + title)
    except IndexError:
        speak("Sorry, i cannot play it, please try again", save=False)
